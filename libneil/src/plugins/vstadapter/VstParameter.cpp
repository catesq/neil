...
VstParameter* VstParameter::build(VstParameterProperties* properties, zzub::parameter* zzub_param, uint16_t index) {
    if(!properties)
        return new VstFloatParameter(properties, zzub_param, index);
    else if(properties->flags & kVstParameterIsSwitch)
        return new VstSwitchParameter(properties, zzub_param, index);
    else if (properties->flags & kVstParameterUsesIntegerMinMax && !(properties->flags & kVstParameterUsesFloatStep))
        return new VstIntParameter(properties, zzub_param, index);
    else
        return new VstFloatParameter(properties, zzub_param, index);
}
...
VstParameter::VstParameter(VstParameterProperties* vst_props,
...
VstIntParameter::VstIntParameter(VstParameterProperties* vst_props,
                                 zzub::parameter* zzub_param,
                                 uint16_t index) : VstParameter(vst_props, zzub_param, index)
{
...
VstFloatParameter::VstFloatParameter(VstParameterProperties* vst_props,
                                     zzub::parameter* zzub_param,
                                     uint16_t index) :
    VstParameter(vst_props, zzub_param, index),
    min(0),
    max(1)
{
