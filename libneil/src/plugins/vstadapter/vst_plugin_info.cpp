...
vst_zzub_info::vst_zzub_info(AEffect* plugin,
    ...
    switch (category) {
        ...
        case kPlugCategGenerator:
            printf("Plugin '%s' is a generator\n", name.c_str());
            min_tracks = 1;
            max_tracks = 16;

            // add several track paramaters: note, volume, two midi commands to this->track_parameters 
            zzub::midi_track_manager::add_midi_track_info(this);

            flags |= (zzub::plugin_flag_is_instrument | zzub::plugin_flag_has_audio_output | zzub::plugin_flag_is_instrument | zzub::plugin_flag_has_midi_input);
            break;
...
int vst_zzub_info::get_param_count() const {
