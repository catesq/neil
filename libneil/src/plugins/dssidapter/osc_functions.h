...
int osc_midi_handler(struct dssidapter *instance, lo_arg **argv) {
    ...
    count = snd_midi_event_encode(alsaCoder, (argv[0]->m) + 1, 3, alsaEncodeBuffer); /* ignore OSC "port id" in argv[0]->m[0] */
...
int osc_control_handler(struct dssidapter *instance, lo_arg **argv) {
...
int osc_program_handler(struct dssidapter *instance, lo_arg **argv) {
    int bank = argv[0]->i;
...
int osc_configure_handler(struct dssidapter *instance, lo_arg **argv) {
...
int osc_update_handler(struct dssidapter *instance, lo_arg **argv, lo_address source) {
...
int osc_exiting_handler(struct dssidapter *instance, lo_arg **argv) {
    if (verbose) {
