...
void zzub_player_blacklist_plugin(
...
void zzub_player_add_plugin_alias(
    zzub_player_t* player,
    const char* alias,
    const char* uri
)
...
void zzub_player_add_plugin_path(
    zzub_player_t* player,
    const char* path
)
{
...
void zzub_player_history_commit(
    zzub_player_t* player,
...
int zzub_player_history_get_uncomitted_operations(
    zzub_player_t* player
)
{
...
void zzub_player_history_flush(
    zzub_player_t* player
)
{
    player->flush_operations(0, 0, 0);
...
void zzub_player_history_flush_last(
    zzub_player_t* player
)
{
    player->flush_operations(0, 0, 0);
...
int zzub_player_history_get_size(
    zzub_player_t* player
)
{
...
int zzub_player_history_get_position(
    zzub_player_t* player
)
{
...
zzub_plugincollection_t* zzub_plugincollection_get_by_uri(
    zzub_player_t* player,
    const char* uri
)
{
...
void zzub_plugincollection_configure(
...
zzub_pluginloader_t* zzub_player_get_pluginloader(
    zzub_player_t* player,
    int index
)
{
...
zzub_pluginloader_t* zzub_player_get_pluginloader_by_name(
    zzub_player_t* player, 
    const char* name
)
{
...
int zzub_player_load_ccm(
    zzub_player_t* player, 
    const char* fileName
)
{
    CcmReader f;
    if (!f.open(fileName, player))
        return -1;
...
int zzub_player_save_ccm(
    zzub_player_t* player, 
    const char* fileName
)
{
    CcmWriter f;
    if (!f.save(fileName, player))
        return -1;
    return 0;
}


int zzub_player_save_ccm_selected(
    zzub_player_t* player, 
    const char* fileName, 
    const int* plugins, 
    unsigned int size
)
{
    ...
    if (!f.saveSelected(fileName, player, plugins, size))
        return -1;
    return 0;
}


int zzub_player_get_state(
    zzub_player_t* player
)
{
...
void zzub_player_set_state(
    zzub_player_t* player, int state
)
{
...
int zzub_player_get_plugin_count(
    zzub_player_t* player
)
{
...
void zzub_player_get_new_plugin_name(
    zzub_player_t* player, 
    const char* uri, 
    char* name, 
    int maxLen
...
zzub_plugin_t* zzub_player_get_plugin_by_name(
    zzub_player_t* player, 
    const char* name
)
{
    ...
    if (plugindesc == graph_traits<plugin_map>::null_vertex())
        return 0;
...
zzub_plugin_t* zzub_player_get_plugin_by_id(
    zzub_player_t* player, 
    int id
)
{
...
zzub_plugin_t* zzub_player_get_plugin(
    zzub_player_t* player, 
    int index
)
{
...
int zzub_plugin_set_midi_connection_device(
...
void zzub_plugin_add_event_connection_binding(
    zzub_plugin_t* to_plugin, 
    zzub_plugin_t* from_plugin, 
    int sourceparam, 
    int targetgroup, 
...
void zzub_plugin_add_cv_connector(
    zzub_plugin_t* to_plugin, 
    zzub_plugin_t* from_plugin, 
    zzub_cv_node_t* source,
    zzub_cv_node_t* target, 
    zzub_cv_connector_opts_t* opts
)
{
    to_plugin->_player->plugin_add_cv_connector(to_plugin->id, from_plugin->id, zzub::cv_connector(*source, *target, *opts));
}


void zzub_plugin_remove_cv_connector(
    zzub_plugin_t* to_plugin, 
    zzub_plugin_t* from_plugin, 
    int subconn_id
)
...
zzub::cv_connector* zzub_plugin_get_cv_connector(
    zzub_plugin_t* to_plugin,
    zzub_plugin_t* from_plugin,
    int subconn_id
)
...
void zzub_plugin_update_cv_connector(
    zzub_plugin_t* to_plugin,
    zzub_plugin_t* from_plugin,
    zzub_cv_node_t* source,
    zzub_cv_node_t* target,
...
)
{
    ...
    zzub::cv_connector new_connector(*source, *target, *opts);
...
const float** zzub_player_work_stereo(
    zzub_player_t* player, 
    int* numSamples
)
{
...
int zzub_player_get_loop_start(
    zzub_player_t* player
)
{
...
int zzub_player_get_song_start(
    zzub_player_t* player
)
{
...
void zzub_player_set_loop_start(
    zzub_player_t* player, 
    int v
)
{
...
void zzub_player_set_song_start(
    zzub_player_t* player, 
    int v
)
{
...
void zzub_player_set_loop_enabled(
    zzub_player_t* player, 
    int enable
)
{
...
int zzub_player_get_loop_enabled(
    zzub_player_t* player
)
{
...
int zzub_player_get_sequence_track_count(
    zzub_player_t* player
)
{
...
zzub_sequence_t* zzub_player_get_sequence(
    zzub_player_t* player, 
    int index
)
{
...
int zzub_player_get_currently_playing_pattern(
...
int zzub_player_get_currently_playing_pattern_row(
    zzub_plugin_t* plugin, 
    int pattern, 
    int* row
)
...
int zzub_player_get_wave_count(
    zzub_player_t* player
)
{
...
zzub_wave_t* zzub_player_get_wave(
    zzub_player_t* player, int index
)
{
...
zzub_event_data_t* zzub_player_get_next_event(
    zzub_player_t* player
)
{
...
void zzub_player_handle_events(
    zzub_player_t* player
)
{
...
void zzub_player_set_event_queue_state(
    zzub_player_t* player, 
    int enable
)
{
...
zzub_midimapping_t* zzub_player_add_midimapping(
    zzub_plugin_t* plugin, 
    int group, 
    int track, 
    int param, 
...
int zzub_player_remove_midimapping(
    zzub_plugin_t* plugin, 
    int group, 
    int track, 
    int param
...
zzub_midimapping_t* zzub_player_get_midimapping(
    zzub_player_t* player, 
    int index
)
{
...
void zzub_player_set_midi_plugin(zzub_player_t* player, zzub_plugin_t* plugin)
{
    if (plugin == 0) {
        ...
        if (plugin->id >= player->front.plugins.size() || player->front.plugins[plugin->id] == 0)
            return;
...
zzub_plugin_t* zzub_player_create_plugin(zzub_player_t* player, zzub_input_t* input, int dataSize, const char* name, zzub_pluginloader_t* loader, int flags)
{
    ...
    if (dataSize > 0)
        input->read(&bytes.front(), dataSize);
...
int zzub_plugin_get_pattern_index(zzub_plugin_t* plugin, zzub_pattern_t* pattern)
{
    ...
    for (size_t i = 0; i < plugin->_player->back.plugins[plugin->id]->patterns.size(); i++)
        if (plugin->_player->back.plugins[plugin->id]->patterns[i] == pattern)
            return (int)i;
    return -1;
}
...
int zzub_plugin_get_pattern_by_name(zzub_plugin_t* plugin, const char* name)
{
    ...
    plugin->_player->merge_backbuffer_flags(flags);
    for (size_t i = 0; i < plugin->_player->back.plugins[plugin->id]->patterns.size(); i++)
        if (plugin->_player->back.plugins[plugin->id]->patterns[i]->name == name)
            return (int)i;
    return -1;
}
...
zzub_cv_connector_opts_t* zzub_cv_connector_opts_create()
{
    return new zzub::cv_connector_opts();
}


zzub_cv_node_t* zzub_cv_connector_get_source(zzub_cv_connector_t* connector)
{
    return static_cast<zzub_cv_node_t*>(&connector->source_node);
}


zzub_cv_node_t* zzub_cv_connector_get_target(zzub_cv_connector_t* connector)
{
    return static_cast<zzub_cv_node_t*>(&connector->target_node);
}


zzub_cv_connector_opts_t* zzub_cv_connector_get_opts(zzub_cv_connector_t* connector)
{
    return static_cast<zzub_cv_connector_opts_t*>(&connector->opts);
}
...
{
    switch (group) {
    case zzub_parameter_group_connection:
        return zzub_plugin_get_input_connection_count(plugin);
    case zzub_parameter_group_global:
        return 1;
    case zzub_parameter_group_track:
        return zzub_plugin_get_track_count(plugin);
    default:
        return 0;
    }
    return 0;
...
int zzub_plugin_describe_value(zzub_plugin_t* plugin, int group, int column, int value, char* name, int maxlen)
{
    if (group == 0) {
        if (column == 0) {
            ...
            if (dB > -100)
                strm << std::setprecision(2) << std::fixed << dB << " dB";
            else
                strm << "-inf dB";
...
{
    if (plugin->id >= plugin->_player->front.plugins.size() || plugin->_player->front.plugins[plugin->id] == 0)
        return 0;
...
int zzub_plugin_linear_to_pattern(zzub_plugin_t* plugin, int index, int* group, int* track, int* column)
{
    ...
    if (!info->track_parameters.size())
        return 0;

    int t = index / info->track_parameters.size();
    if (t >= plugin->_player->back.plugins[plugin->id]->tracks)
        return 0;
...
int zzub_sequence_get_event_at(zzub_sequence_t* sequence, int pos)
{
    ...
    if (song_index >= event_count || pos != timestamp)
        return -1;
...
void zzub_plugin_get_new_pattern_name(zzub_plugin_t* plugin, char* name, int maxLen)
{
    ...
    for (int i = zzub_plugin_get_pattern_count(plugin); i < 9999; i++) {
        if (i < 100)
            strm << std::setw(2) << std::setfill('0') << i;
        else if (i < 1000)
            strm << std::setw(3) << std::setfill('0') << i;
        else if (i < 10000)
            strm << std::setw(4) << std::setfill('0') << i;
        else if (i < 100000)
        ...
        for (int i = 0; i < digestsize; ++i) {
            ...
            for (int s = (int)blockstart; s < (int)blockend; ++s) {
                ...
                case 3:
                    ...
                    if (isample & 0x00800000)
                        isample = isample | 0xFF000000;
                    break; // TODO
                case 4:
                    switch (format) {
                    case wave_buffer_type_si16:
